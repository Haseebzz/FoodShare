...
const RecipeInfoPage = () => {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await fetch('https://web103-finalproject.vercel.app/comments', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      const response = await fetch(`https://web103-finalproject.vercel.app/comments/${commentId}`, {
        method: 'DELETE',
      });
  ...
  const addRecipeToCollection = async (recipeId, collectionId) => {
    try {
      const response = await fetch(`https://web103-finalproject.vercel.app/collections/${collectionId}/${recipeId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleAddToCollection = async (recipeId) => {
    try {
      const response = await fetch(`https://web103-finalproject.vercel.app/collections/${window.localStorage.userID}`);
      if (response.ok) {
